  ...
  productBundles: [
    {
      ...
      products: ["#internet", "#tv"],
      freeProducts: ["#decoder"],
      prices: [
        { year: 2023, price: 79 },
        { year: 2024, price: 89 },
        { year: 2025, price: 99 },
...
function validateRequestedProduct(db: Database, requestedProducts: ProductId[]): ProductsValidationResult {
  ...
  for (const productId of requestedProducts) {
    try {
      ...
      const errorMessage =
        error instanceof UnknownProductError || error instanceof MissingRequiredProductError
          ? error.message
...
export function prepareOffer(selectedProducts: ProductId[], year: Year): Offer {
